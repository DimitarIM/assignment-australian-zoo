...
function AnimalTab({setPrevIndex, prevIndex, setActiveIndex, activeIndex, animal, location }) {
    const [isActiveState, setActiveState] = useState(false);
    
    useEffect(() => {
        if (animal.id !== activeIndex) setActiveState(false);
    })
    ...
    return (
        <>
            <Link onClick={() => {
                ...
                <img src={`/img/icons/${animal.iconFilename}`} alt="" /></Link>
        </>
